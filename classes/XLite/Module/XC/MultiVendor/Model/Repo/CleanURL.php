...
class CleanURL extends \XLite\Model\Repo\CleanURL implements \XLite\Base\IDecorator
{
    ...
    public static function getEntityTypes()
    {
        ...
        $list['XLite\Model\Profile'] = 'vendor';

        return $list;
    }

    /**
     * Returns vendor url regexp pattern
     *
     * @return string
     */
    protected function getPatternVendor()
    {
     ...
     * @return string
     */
    protected function getBaseFieldNameVendor($entity)
    {
     ...
     * @return string
     */
    protected function postProcessURLVendor($url, $entity)
    {
     ...
     * @return array
     */
    protected function parseURLVendor($url, $last = '', $rest = '', $ext = '')
    {
        $result = null;

        if ($ext) {
            $result = $this->findByURL('vendor', $url . $ext);
        }
    ...
    protected function prepareParseURL($url, $last, $rest, $ext, $target, $params)
    {
        ...
        if ('vendor' === $target) {
            if (isset($params['profile_id'])) {
                $params['vendor_id'] = $params['profile_id'];
            }
     ...
     * @return array
     */
    protected function buildURLVendor($params)
    {
        $urlParts = array();

        $id = isset($params['vendor_id'])
            ? $params['vendor_id']
            : (isset($params['profile_id']) ? $params['profile_id'] : null);
        if ($id) {
            /** @var \XLite\Model\Profile $vendor */
            $vendor = \XLite\Core\Database::getRepo('XLite\Model\Profile')->find($id);

            if (null !== $vendor && $vendor->getCleanURL()) {
                $urlParts[] = $vendor->getCleanURL();
                unset($params['vendor_id']);
                unset($params['profile_id']);
            }
        }
     ...
     * @return string
     */
    protected function buildFakeURLVendor($entity, $params)
    {
